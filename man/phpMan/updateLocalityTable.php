...
if(isset($d->id) && $d->id != NULL){
    $result = mysql_query(sprintf("UPDATE locality SET name = '%s', characteristics = '%s'"
    ...
    $result = mysql_query(sprintf("INSERT INTO locality(name,characteristics,charact_en,charact_fr,charact_be)"
            . " VALUES('%s','%s','%s','%s','%s')",
            $d->name,$d->characteristics,$d->charact_en,$d->charact_fr,$d->charact_be));
   if($result){
        $data = array();

        $result = mysql_query("SELECT * FROM locality");
